...
class heartClockViewController: UIViewController,UNUserNotificationCenterDelegate {
    ...
    override func viewDidLoad() {
        ...
        UNUserNotificationCenter.current().requestAuthorization(options: [.alert, .sound, .badge,]) { (granted, error) in
            ...
            if granted {
                allow = true
                UserDefaults.standard.set(allow, forKey: "notificationsSwitch")
            } else  {
        ...
        case "iphone5-5s-5c-SE": //iPhone5-5s-5c-SE
            heartLoadingView.frame = CGRect(x: 0, y: 0, width: 170, height: 170)
            heartLoadingView.center = self.view.center
            heartLoadingView.center.y = self.view.center.y+20
        case "iphone678": //iPhone6-7-8
            heartLoadingView.frame = CGRect(x: 0, y: 0, width: 230, height: 230)
            heartLoadingView.center = self.view.center
            heartLoadingView.center.y = self.view.center.y+7
        case "iphone678-Plus": //iPhone6-7-8-Plus
            heartLoadingView.frame = CGRect(x: 0, y: 0, width: 270, height: 270)
            heartLoadingView.center = self.view.center
        case "Main": //iPhoneX-XS
            heartLoadingView.frame = CGRect(x: 0, y: 0, width: 270, height: 270)
            heartLoadingView.center = self.view.center
        case "iphoneXR-XSmax": //iPhoneXR-XSmax
            heartLoadingView.frame = CGRect(x: 0, y: 0, width: 330, height: 330)
            heartLoadingView.center = self.view.center
            heartLoadingView.center.y = self.view.center.y-10
        default:
            break
        }
    ...
    func messageUpdate() {
        ...
        if messageArray.isEmpty == false {
            ...
            let num = Int(arc4random()) % messageArray.count
            
            topicDateLabel.text = messageArray[num].date + "のあなたからのメッセージ"
            topicMessageLabel.text = messageArray[num].message
            
        } else {
            
            topicDateLabel.text = ""
            topicMessageLabel.text = "メッセージがまだありません。\n下の「＋」ボタンから記録しましょう！"
        }
        ...
        topicMessageLabel.attributedText = NSMutableAttributedString(string: topicMessageLabel.text!, attributes: lineSpaceAttr)
        topicMessageLabel.textColor = UIColor.gray
